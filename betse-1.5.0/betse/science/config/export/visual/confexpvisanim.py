...
# ....................{ SUBCLASSES                        }....................
class SimConfExportAnims(YamlABC):
    '''
    ...
    is_overlay_current : bool
        ``True`` only if this configuration overlays streamlines (e.g.,
        electric current, concentration flux) onto appropriate animations.
    ...
    is_after_sim_show : bool
        ``True`` only if this configuration displays post-simulation
        animations.
    ...
    video_metadata : MappingType
        ...
        ``copyright``, ``srcform``, and ``comment``. If this dictionary does
        *not* contain a ``copyright`` key, this key will be automatically
        synthesized from the current year. Ignored if :attr:`is_video_save` is
        ``False``.
