...
'''
Create and electrodiffuses a suite of customizable general molecule in the
BETSE ecosystem, including functionality to pump the molecule, use it as a
gating ligand, produce and consume it, and use it an enzyme to facilitate
another reaction.
...
# --------------------( PLOTTERS                          )--------------------
#FIXME: Ideally, the "self" parameter should be specific to class methods.
#Since these are simple functions with no parent objects, we should probably
#rename all "self" parameters in this submodule to... well, something. Since
#"self" appears to refer to an instance of the "MasterOfGenes" or
#"MasterOfNetworks" classes, "master" might be a suitable parameter name. Yum!
...
def plot_master_network(self, p):
    ...
    for i, name in enumerate(self.molecules):
        ...
        if mol.simple_growth:
            ...
            graph_influencers(
                self, base_graph, gname, mol.growth_activators_list,
        ...
        if mol.ion_channel_gating:
            graph_influencers(
                self, base_graph, mol.gating_channel_name, mol.ion_activators_list,
    ...
    base_graph.add_edge(pydot.Edge(
        'Pmem_K', 'Vmem', arrowhead='tee', color='red', penwidth=self.edge_width))
    base_graph.add_edge(pydot.Edge(
        'K_env', 'Vmem', arrowhead='dot', color='blue', penwidth=self.edge_width))
    ...
    if len(self.reactions_env) > 0:
        for i, name in enumerate(self.reactions_env):
            ...
            graph_influencers(
                self, base_graph, name, rea.reaction_activators_list,
    ...
    if len(self.transporters) > 0:
        ...
        for name in self.transporters:
            ...
            graph_influencers(
                self, base_graph, name, trans.transporter_activators_list,
    ...
    if len(self.channels) > 0:
        for i, name in enumerate(self.channels):
            ...
            graph_influencers(
                self, base_graph, name, chan.channel_activators_list,
...
def make_subgraphs(self, base_graph, p):
    ...
    for i, sn_dic in enumerate(subnetworks):
        sn_py = write_nodes(
            subnetworks[sn_dic]['nx subnet'], subnetworks[sn_dic]['py subnet'])
...
def graph_influencers(
    ...
    if a_list != 'None' and a_list is not None:
        for act_name, zone_a in zip(a_list, zone_tags_a):
            ...
            base_graph.add_edge(pydot.Edge(
                act_name, name, penwidth=self.edge_width, arrowhead='dot', color='blue'))
    ...
    if i_list != 'None' and i_list is not None:
        for inh_name, zone_i in zip(i_list, zone_tags_i):
            ...
            base_graph.add_edge(pydot.Edge(
                inh_name, name, arrowhead='tee', color='red', penwidth=self.edge_width))
